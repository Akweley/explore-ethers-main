...
// Define  function
async function findAddresses(senderAddress) {
  const recipientAddresses = new Set(); // handle uniqueness
  //ensures that each sender addresss is stored only once regardless of how many transactions meet the requiremwnts in the loop.

  const latestBlockNumber = await provider.getBlockNumber();

  for (let blockNumber = 0; blockNumber <= latestBlockNumber; blockNumber++) {
    const block = await provider.getBlockWithTransactions(blockNumber);
    console.log(`Block #${blockNumber}, Transaction count: ${block.transactions.length}`);
    
    block.transactions.forEach(transaction => {
    console.log(`Transaction - from: ${transaction.from}, to: ${transaction.to}`);
      if (transaction.to === senderAddress.toLowerCase()) {//to avoid errors from case sensitivity 
        recipientAddresses.add(transaction.from.toLowerCase());
        //where transaction.to == reciepient address 
  ...
  return Array.from(recipientAddresses);
}

// Specified address from which you want to find the recipients
(async () => {
  console.log("balance wallet1: ", utils.formatEther(await wallet1.getBalance()));
  console.log("balance wallet2: ", utils.formatEther(await wallet2.getBalance()));
  console.log("balance wallet3: ", utils.formatEther(await wallet3.getBalance()));

  const specifiedAddress = wallet1.address; // Use the address you want to search from
  const recipientAddresses = await findAddresses(specifiedAddress);

  console.log(`List of addresses that received ether from ${specifiedAddress}:`, recipientAddresses);
})();